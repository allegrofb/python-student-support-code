...
class Compiler:
    ...
    def shrink_stmt(self, s: stmt) -> stmt:
        ...
        match s:
            ...
            case AnnAssign(var, typ, exp):
                return AnnAssign(var, typ, self.shrink_exp(exp))
            case _:
    ...
    def uniquify_exp(self, e: expr, var_refs: Set) -> expr:
        # YOUR CODE HERE
        match e:
            case Name(n):
                if n in var_refs.keys():
                    return Name(n+"_"+str(var_refs[n]))
                return Name(n)
            case BinOp(left, Add(), right):
                return BinOp(self.uniquify_exp(left, var_refs), Add(), self.uniquify_exp(right, var_refs))
            case BinOp(left, Sub(), right):
                return BinOp(self.uniquify_exp(left, var_refs), Sub(), self.uniquify_exp(right, var_refs))
            case UnaryOp(USub(), v):
                return UnaryOp(USub(), self.uniquify_exp(v, var_refs))
            case IfExp(test, body, orelse):
                return IfExp(self.uniquify_exp(test, var_refs), self.uniquify_exp(body, var_refs), self.uniquify_exp(orelse, var_refs))
            case UnaryOp(Not(), v):
                return UnaryOp(Not(), self.uniquify_exp(v, var_refs))
            case BoolOp(And(), values):
                return UnaryOp(And(), self.uniquify_exp(v, var_refs))
            case BoolOp(Or(), values):
                return BoolOp(Or(), [self.uniquify_exp(i, var_refs) for i in values])
            case Compare(left, [cmp], [right]):
                return Compare(self.uniquify_exp(left, var_refs), [cmp], [self.uniquify_exp(right, var_refs)])
            case Call(func, args):
                return Call(self.uniquify_exp(func, var_refs), [self.uniquify_exp(i, var_refs) for i in args])
            case Constant(v):
                return Constant(v)
            case ast.Tuple(es, Load()):
                return ast.Tuple([self.uniquify_exp(i, var_refs) for i in es], Load())
            case Subscript(tup, Constant(index), Load()):
                return Subscript(self.uniquify_exp(tup, var_refs), Constant(index), Load())
            case Lambda(var, exp):
                tmp = {}
                tmp_var = []
                for i in var:
                    if i in var_refs.keys():
                        var_refs[i] += 1
                        while(i+"_"+str(var_refs[i]) in var_refs.keys()):
                            var_refs[i] += 1
                        tmp[i] = var_refs[i]
                        tmp_var.append(i+"_"+str(var_refs[i]))
                    else:
                        tmp_var.append(i)
                return Lambda(tmp_var, self.uniquify_exp(exp, tmp))
            case _:
                raise Exception('error in uniquify_exp, unexpected ' + repr(e))

    def uniquify_stmt(self, s: stmt, var_refs: Dict) -> stmt:
        # YOUR CODE HERE
        match s:
            case If(test, body, orelse):
                return If(self.uniquify_exp(test, var_refs), 
                            [self.uniquify_stmt(i, var_refs) for i in body], 
                            [self.uniquify_stmt(i, var_refs) for i in orelse])
            case Assign([Name(name)], value):
                if name not in var_refs.keys():
                    var_refs.update({name:0})
                return Assign([Name(name)], self.uniquify_exp(value, var_refs))
            case Expr(Call(Name('print'), [arg])):
                return Expr(Call(Name('print'), [self.uniquify_exp(arg, var_refs)]))
            case Expr(value):
                return Expr(self.uniquify_exp(value, var_refs))
            case While(test, body, []):
                return While(self.uniquify_exp(test, var_refs),
                          [self.uniquify_stmt(i, var_refs) for i in body], [])
            case Return(value):
                return Return(self.uniquify_exp(value, var_refs))
            case AnnAssign(var, typ, exp):
                return AnnAssign(var, typ, self.uniquify_exp(exp, var_refs))
            case _:
                raise Exception('error in uniquify_stmt, unexpected ' + repr(e))
                ...
                for stmt in body:
                    match stmt:
                        case FunctionDef(name, params, bod, dl, returns, comment):
                            var_refs = {}
                            for p in params:
                                var_refs.update({p[0]:0})
                            new_bod = []
                            for s in bod:
                                new_bod.append(self.uniquify_stmt(s, var_refs))
                            new_body.append(FunctionDef(name, params, new_bod, dl, returns, comment))
                        case _:
                            raise Exception('error in uniquify, unexpected ' + repr(stmt))
                print(new_body)
                return Module(new_body)
            case _:
                raise Exception('error in uniquify, unexpected ' + repr(p))
    ...
    def reveal_exp(self, e: expr, func_refs: Dict) -> expr:
        ...
        match e:
            ...
            case Lambda(var, exp):
                return Lambda(var, self.uniquify_exp(exp, func_refs))
            case _:
    ...
    def reveal_stmt(self, s: stmt, func_refs: Dict) -> stmt:
        ...
        match s:
            ...
            case AnnAssign(var, typ, exp):
                return AnnAssign(var, typ, self.reveal_exp(exp, func_refs))
            case _:
    ...
    ############################################################################
    def free_variables(self, e:expr) -> List[Name]:
        pass

    def free_in_lamda(self, func:stmt) -> List[Name]:
        pass

    def assigned_vars_stmt(self, func:stmt) -> List[Name]:
        match stmt:
            case FunctionDef(name, params, bod, dl, returns, comment):
                for s in bod:
                    match s:
                        case Assign([Name(id)], value):
                            return Assign([Name(id)], self.convert_exp(value, var_refs))
                        case While(test, body, []):
                            return While(self.convert_exp(test, var_refs),
                                    [self.convert_stmt(i, var_refs) for i in body], [])
                        case AnnAssign(var, typ, exp):
                            return AnnAssign(var, typ, self.convert_exp(exp, var_refs))
            case _:
                raise Exception('error in convert_assignments, unexpected ' + repr(stmt))

    def convert_exp(self, e: expr, var_refs: Set) -> expr:
        # YOUR CODE HERE
        match e:
            case Name(n):
                if n in var_refs:
                    return FunRef(n, var_refs[n])
                return Name(n)
            case BinOp(left, Add(), right):
                return BinOp(self.convert_exp(left, var_refs), Add(), self.convert_exp(right, var_refs))
            case BinOp(left, Sub(), right):
                return BinOp(self.convert_exp(left, var_refs), Sub(), self.convert_exp(right, var_refs))
            case UnaryOp(USub(), v):
                return UnaryOp(USub(), self.convert_exp(v, var_refs))
            case IfExp(test, body, orelse):
                return IfExp(self.convert_exp(test, var_refs), self.convert_exp(body, var_refs), self.convert_exp(orelse, var_refs))
            case UnaryOp(Not(), v):
                return UnaryOp(Not(), self.convert_exp(v, var_refs))
            case BoolOp(And(), values):
                return UnaryOp(And(), self.convert_exp(v, var_refs))
            case BoolOp(Or(), values):
                return BoolOp(Or(), [self.convert_exp(i, var_refs) for i in values])
            case Compare(left, [cmp], [right]):
                return Compare(self.convert_exp(left, var_refs), [cmp], [self.convert_exp(right, var_refs)])
            case Call(func, args):
                return Call(self.convert_exp(func, var_refs), [self.convert_exp(i, var_refs) for i in args])
            case Constant(v):
                return Constant(v)
            case ast.Tuple(es, Load()):
                return ast.Tuple([self.convert_exp(i, var_refs) for i in es], Load())
            case Subscript(tup, Constant(index), Load()):
                return Subscript(self.convert_exp(tup, var_refs), Constant(index), Load())
            case Lambda(var, exp):
                return Lambda(var, self.uniquify_exp(exp, var_refs))
            case _:
                raise Exception('error in convert_exp, unexpected ' + repr(e))

    def convert_stmt(self, s: stmt, var_refs: Set) -> stmt:
        # YOUR CODE HERE
        match s:
            case If(test, body, orelse):
                return If(self.convert_exp(test, var_refs), 
                            [self.convert_stmt(i, var_refs) for i in body], 
                            [self.convert_stmt(i, var_refs) for i in orelse])
            case Assign([Name(id)], value):
                return Assign([Name(id)], self.convert_exp(value, var_refs))
            case Expr(Call(Name('print'), [arg])):
                return Expr(Call(Name('print'), [self.convert_exp(arg, var_refs)]))
            case Expr(value):
                return Expr(self.convert_exp(value, var_refs))
            case While(test, body, []):
                return While(self.convert_exp(test, var_refs),
                          [self.convert_stmt(i, var_refs) for i in body], [])
            case Return(value):
                return Return(self.convert_exp(value, var_refs))
            case AnnAssign(var, typ, exp):
                return AnnAssign(var, typ, self.convert_exp(exp, var_refs))
            case _:
                raise Exception('error in convert_stmt, unexpected ' + repr(s))
        ...
        match p:
            case Module(body):
                ...
                for stmt in body:
                    match stmt:
                        case FunctionDef(name, params, bod, dl, returns, comment):
                            free_vars = self.free_in_lamda(stmt)
                            assigned_vars = self.assigned_vars_stmt(stmt)
                            af = free_vars & assigned_vars
                            new_bod = []
                            for s in bod:
                                new_bod.append(self.convert_stmt(s, var_refs))
                            new_body.append(FunctionDef(name, params, new_bod, dl, returns, comment))
                        case _:
                            raise Exception('error in convert_assignments, unexpected ' + repr(stmt))
                print(new_body)
                return Module(new_body)
            case _:
                raise Exception('error in convert_assignments, unexpected ' + repr(p))
